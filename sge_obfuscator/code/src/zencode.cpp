...
int main(int argc, char* argv[]){	
	...
	char* ta_id;
  	ta_id = getenv ("SGE_TASK_ID");
	int task_id;	
	if (ta_id != NULL){
		task_id = atoi( ta_id) -1; //to use zero-indexing ; 
	}else{ task_id = atoi(argv[2])-1;}
	...
	if (argc<2){
		fprintf(stderr,"Must specify dir!\n"); 
		exit(4);
	}/* else if (argc >=3){
		dir_secret = argv[2];
	}*/ else {
	...
	s->mlm = load_clt(dir_pub,dir_secret);
	
	if (mpz_cmp_ui(s->mlm->q, 0 ) ==0){
	fprintf(stderr, "\n \n WHY IS Q ZERO? \n \n");
	}
...
char *fname;
	...
	myfile.open (fname, ios::in | ios::binary); 
	myfile.read( (char*)(&NUMSLOTS),sizeof(int));
	myfile.close();
    ...
    (void) snprintf(fname, fnamelen, "%s/deg_%d", s->dir, task_id);

	//get degs

	int degsize = (((task_id+1)*n/NUMSLOTS) - task_id*n/NUMSLOTS);
	int* degs = new int[degsize];

	myfile.open (fname, ios::in | ios::binary); 
	myfile.read( (char*)(degs),sizeof(int)*degsize);
	myfile.close();
	unlink(fname);


	int ysize = (((task_id+1)*m/NUMSLOTS) - task_id*m/NUMSLOTS);
	

	//for(int i = 0; i<degsize; i++){
		//fprintf(stderr, "deg %d: %d \n", i+task_id*n/NUMSLOTS, degs[i]);
//	}


	//get alphas
	mpz_t* alphas = new mpz_t[degsize];
	for (int i = 0; i<degsize; i++){
		mpz_init(alphas[i]);
	}
	(void) snprintf(fname, fnamelen, "%s/a_%d", s->dir,task_id);
	load_mpz_vector(fname,alphas,degsize);
	unlink(fname);

	//get betas
	mpz_t* betas = new mpz_t[ysize];
	for (int i = 0; i<ysize; i++){
		mpz_init(betas[i]);
	}
	(void) snprintf(fname, fnamelen, "%s/b_%d", s->dir,task_id);
	load_mpz_vector(fname,betas,ysize);
	unlink(fname);
	
//get ys
	mpz_t* ys = new mpz_t[ysize];
	for (int i = 0; i<ysize; i++){
		mpz_init(ys[i]);
	}
	(void) snprintf(fname, fnamelen, "%s/ys_%d", s->dir,task_id);
	load_mpz_vector(fname,betas,ysize);
	unlink(fname);
	...
	//do work!
	 ...
	 int idx_set_size = 4 * n + 1;

   int* indices = (int *) malloc(sizeof(int) * idx_set_size);
    int* pows = (int *) malloc(sizeof(int) * idx_set_size);



	int start_place = task_id*n/NUMSLOTS;

	 for (int i = start_place; i < (task_id+1)*n/NUMSLOTS; ++i) {
        mpz_t out, elems[2];
        int deg;
	...
	//fprintf(stderr, "%d, " ,deg);
        ...
        mpz_set(elems[1], alphas[i-start_place]);
	...
	start_place = task_id*m/NUMSLOTS;
    for (int i = start_place; i < (task_id+1)*m/NUMSLOTS; ++i) {
        mpz_t out, elems[2];
        mpz_inits(out, elems[0], elems[1], NULL);

        mpz_set(elems[0], ys[i-start_place]);
        if (mpz_sgn(elems[0]) == -1) {
            mpz_mod(elems[0], elems[0], s->nev);
        }
        mpz_set(elems[1], betas[i-start_place]);
        clt_mlm_encode(s->mlm, out, 2, elems, 1, indices, pows);
