...
obf_encode_circuit(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OsOOiiii", &py_state, &circuit, &py_ys,
                          &py_xdegs, &ydeg, &n, &m,&NUMSLOTS))
        return NULL;
	...
	int fnamelen = strlen(s->dir) + 10;
    fname = (char *) malloc(sizeof(char) * fnamelen+15);
    if (fname == NULL)
        return NULL;
	...
	double intertime, totaltime;

	//!!MR!!!!!!!!!!!!!!

	write_secret_params(&(s->mlm),s->dir);

	intertime=current_time();
	totaltime = intertime - starttime;
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
	...
	//write degs to files MR might have problems if ints different sizes on different systems!
	starttime = current_time();
	ofstream myfile;
	
	for (int i = 0; i< NUMSLOTS; i++){
			 
	 	(void) snprintf(fname, fnamelen, "%s/deg_%d", s->dir,i);
		fprintf(stderr,fname);
		myfile.open (fname, ios::out | ios::app | ios::binary);
		myfile.write( (char*)(&degs[i*n/NUMSLOTS]),sizeof(int)*(((i+1)*n/NUMSLOTS) - i*n/NUMSLOTS));
		myfile.close();
	};
	...
	//write n;
		(void) snprintf(fname, fnamelen, "%s/n", s->dir);
		myfile.open (fname, ios::out | ios::app | ios::binary);
		myfile.write( (char*)(&n),sizeof(int));
		myfile.close();

	//write m;
		(void) snprintf(fname, fnamelen, "%s/m", s->dir);
		myfile.open (fname, ios::out | ios::app | ios::binary);
		myfile.write( (char*)(&m),sizeof(int));
		myfile.close();
	

//write NUMSLOTS;
		(void) snprintf(fname, fnamelen, "%s/numslots", s->dir);
		myfile.open (fname, ios::out | ios::app | ios::binary);
		myfile.write( (char*)(&NUMSLOTS),sizeof(int));
		myfile.close();
	...
	for(int i = 0; i< NUMSLOTS; i++){
		//save alphas
		 (void) snprintf(fname, fnamelen, "%s/a_%d", s->dir,i);
		//fprintf(stderr,fname); fprintf(stderr,"\n"); fprintf(stderr,s->dir);fprintf(stderr,"\n");
		save_mpz_vector(fname, &alphas[(i*n/NUMSLOTS)], (i+1)*n/NUMSLOTS - (i*n/NUMSLOTS));
	}


	//save betas
	for(int i = 0; i< NUMSLOTS; i++){
		 (void) snprintf(fname, fnamelen, "%s/b_%d", s->dir,i);
		//fprintf(stderr,fname); fprintf(stderr,"\n"); fprintf(stderr,s->dir);fprintf(stderr,"\n");
		save_mpz_vector(fname, &betas[(i*m/NUMSLOTS)], (i+1)*m/NUMSLOTS - (i*m/NUMSLOTS));
	}
	//save ys
	for(int i = 0; i< NUMSLOTS; i++){
		 (void) snprintf(fname, fnamelen, "%s/ys_%d", s->dir,i);
		//fprintf(stderr,fname); fprintf(stderr,"\n"); fprintf(stderr,s->dir);fprintf(stderr,"\n");
		save_mpz_vector(fname, &ys[(i*m/NUMSLOTS)], (i+1)*m/NUMSLOTS - (i*m/NUMSLOTS));
	}


	const char  *arg_vec[2];
        //char** arg_vec = new char*[2];

	char* home = getenv("HOME");
char* dirstring = (char*)malloc(101);
snprintf(dirstring,100,"%s/obfuscation_grid/code/%s",home,s->dir);
fprintf(stderr,"\n%s\n",dirstring);
	arg_vec[0] = dirstring;
	arg_vec[1] = NULL;

	double endtime = current_time();

	fprintf(stderr, "Writing time: %f\n",endtime-starttime+totaltime);

	launch("/home/mroberts/obfuscation_grid/code/src/launch_zen.sh", arg_vec, NUMSLOTS);
	...
	//fprintf(stderr, "%d, " ,deg);
        ...
        set_indices_pows(indices, pows, 3, 2 * i, degs[i], 2 * n + i, 1, 3 * n + i, 1);
        clt_mlm_encode(&s->mlm, out, 2, elems, 3, indices, pows);
